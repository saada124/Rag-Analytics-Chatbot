...
def analyze_query(query: str) -> dict:
    ...
    try:
        ...
        print(f"[ANALYTICS] Explanation: {pandas_output.explanation}")
        print(f"[ANALYTICS] Code generated:\n{pandas_output.code}")

        # Execute code
        result = execute_pandas_code(pandas_output.code, DATAFRAMES)
        
        # Format result
        formatted = format_result(result)
        formatted["answer"] = f"{pandas_output.explanation}\n\n{formatted['answer']}"
        return formatted
        ...
        import traceback
        traceback.print_exc()
        return {"answer": f"Failed to execute calculation: {str(e)}", "data": []}
